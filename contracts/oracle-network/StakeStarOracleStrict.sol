...
contract StakeStarOracleStrict is IOracleNetwork, Initializable, AccessControlUpgradeable {
    ...
    function save(
        ...
        emit Proposed(epoch, totalBalance, uint32(1) << (uint32(23) + oracleNo));

        uint32 confirmations = 1;
        for (uint32 no = 1; no <= ORACLES_COUNT_MAX; ++no) {
            if (no == oracleNo) continue;

            if (_oracleProposal[no].next_epoch == epoch &&
