...
const OPERATOR_IDS: { [chainId: number]: number[] } = {
  5: [24, 312, 335, 50]
};

const OPERATOR_PUBLIC_KEYS: { [chainId: number]: string[] } = {
  5: [
...
export function operatorIdsFor(chainId: number) {
  if (Object.keys(OPERATOR_IDS).includes(chainId.toString())) {
...
export function operatorPublicKeysFor(chainId: number) {
  if (Object.keys(OPERATOR_PUBLIC_KEYS).includes(chainId.toString())) {
